...
{ 
  ...
  char sql[1024];
  sprintf(sql, "INSERT INTO user(name, passwd, balance) VALUES ('%s', '%s', %d)"
	  , u_name
	  , u_passwd
	  , DEFAULT_BALANCE);
...
int Db_readBalance (const char *name)
{
  if(DEBUG)
  ...
  int balance = atoi ((result+1)[column-1]);
  sqlite3_free_table(result);
  //Db_exec(db, sql);
  sqlite3_close(db);
  return balance;
		     ...
		     , int balance)
{
  ...
  char sql[1024];
  char *errorMsg;
  sprintf(sql, "UPDATE user set balance=%d where name= '%s'", balance, name);
